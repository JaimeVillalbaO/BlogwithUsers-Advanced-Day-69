...
from hashlib import md5 #para hacer compateible gravatar
...
def load_user(user_id):
    return db.get_or_404(UserDatabase, user_id)
...
class BlogPost(db.Model):
    ...
    author = relationship("UserDatabase", back_populates="posts")        
...
class Comment(db.Model):
    ...
    author_id: Mapped[int] = mapped_column(Integer, db.ForeignKey("users.id"))
    comment_author = relationship("UserDatabase", back_populates="comments")    
        #***************Child Relationship*************#
...
def login():
    ...
    if form.validate_on_submit():
        ...
        result = db.session.execute(db.select(UserDatabase).where(UserDatabase.email==email))
        user = result.scalar()
        if not user:
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()            
    return render_template("post.html", post=requested_post, current_user=current_user, form= form, gravatar_url=gravatar_url)
